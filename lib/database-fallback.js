...
// Simple in-process mutex to serialize writes in tests/dev
let writeLock = Promise.resolve();
function withLock(fn) {
  const start = writeLock
    .then(() => fn())
    .catch((e) => {
      throw e;
    });
  writeLock = start.catch(() => {});
  return start;
}

// Database operations with atomic writes and basic concurrency control
export const database = {
  // Marker to identify fallback DB in higher layers
  isFallback: true,
  // Read all data
  read() {
    try {
      ...
      // Attempt automatic repair (truncate to minimal valid structure)
      const repaired = {
        authorizedSources: [],
        mcpServers: [],
        threatIndicators: [],
        ...
        performanceMetrics: [],
        alerts: [],
        metadata: { repairedAt: new Date().toISOString(), version: "2.0.0" },
      };
      try {
        if (!fs.existsSync(DATA_DIR)) {
          fs.mkdirSync(DATA_DIR, { recursive: true });
        }
        fs.writeFileSync(DB_FILE, JSON.stringify(repaired, null, 2));
      } catch {}
      return repaired;
    }
  },
  ...
  write(data) {
    try {
      ...
      // Serialize write operations to avoid partial writes under concurrency
      return withLock(() => {
        if (!fs.existsSync(DATA_DIR)) {
          fs.mkdirSync(DATA_DIR, { recursive: true });
        }
        fs.writeFileSync(DB_FILE, payload, { flag: "w" });
        return true;
      });
    } catch (error) {
  ...
  cleanup() {
    ...
    if (
      ...
      data.performanceMetrics.length > maxEntries
    ) {
