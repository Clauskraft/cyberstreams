...
class SearchService {
  ...
  async unifiedSearch(query, options) {
    const { limit, filters, includeVector, includeSemantic } = options;
    const results = [];
    ...
    const [knowledgeResults, intelligenceResults, agentResults, sourceResults] =
  ...
  async searchIntelligence(query, options) {
    ...
    try {
      ...
      if (!db.prepare) {
        logger.warn("Database not available, returning empty intelligence results");
        return [];
      }
  ...
  async searchSources(query, options) {
    ...
    try {
      ...
      if (!db.prepare) {
        logger.info("Database not available, using fallback authorized sources");
