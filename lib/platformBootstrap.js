...
export default function createPlatformBootstrap() {
  ...
  async function standardizeEnv(env = process.env) {
    ...
    // CTI defaults (dev-friendly)
    setDefault("MISP_BASE_URL", "http://localhost:6666");
    setDefault("MISP_VERIFY_TLS", "false");
    setDefault("OPENCTI_API_URL", "http://localhost:4000");
  ...
  async function ensureOllamaModels({ allowPull = true } = {}) {
    ...
    if (allowPull) {
      await admin.ensureModelsInstalled(models);
    }
    // Always return current model list so UI can show correct count
    const installed = await admin.listModels();
    return {
      reachable: true,
      results: installed.map((m) => ({ name: m.name || m.model, ok: true })),
    };
  }
