...
const FALLBACK_FINDINGS = [
  {
    id: 'fallback-misp-1',
    title: 'CFCS Advisory: Active exploitation of Danish energy providers',
    description:
      'CFCS reports increased intrusion attempts targeting Danish energy distribution systems with spear-phishing and credential theft.',
    source: 'CFCS.dk',
    sourceDomain: 'cfcs.dk',
    url: 'https://cfcs.dk/advisories/dk-energy',
    timestamp: new Date(Date.now() - 45 * 60 * 1000).toISOString(),
    category: ['critical-infrastructure', 'danmark'],
    severity: 'high',
    verified: true,
    tags: ['energy', 'critical infrastructure', 'phishing'],
    cves: [],
    iocs: ['185.199.120.45'],
    origin: 'fallback',
    confidence: 75
  ...
  {
    id: 'fallback-opencti-1',
    title: 'OpenCTI: Observable domain linked to ransomware deployment',
    description: 'Detected malicious domain delivering LockBit payloads across Nordic manufacturing targets.',
    source: 'OpenCTI',
    sourceDomain: 'opencti.internal',
    url: null,
    ...
    cves: [],
    iocs: ['lockbit-update[.]com'],
    origin: 'fallback',
    confidence: 85
  },
  {
    id: 'fallback-misp-2',
    title: 'CERT-EU: APT28 phishing kit targeting ministries',
    description:
      'APT28 spear phishing kit detected with references to Danish ministries. Payload contains macro-enabled documents communicating to .onion infrastructure.',
    source: 'CERT-EU',
    sourceDomain: 'cert.europa.eu',
    url: 'https://cert.europa.eu/publications/apt28-campaign',
    timestamp: new Date(Date.now() - 4 * 60 * 60 * 1000).toISOString(),
    category: ['apt28', 'government'],
    severity: 'high',
    verified: true,
    tags: ['APT28', 'phishing'],
    cves: ['CVE-2024-4587'],
...
export class IntelScraperService {
  constructor({ mispClient, openCtiClient, loadAuthorizedSources, pollIntervalMs = 15 * 60 * 1000 }) {
    ...
    this.latestFindings = [...FALLBACK_FINDINGS]
  ...
  async init() {
    ...
    this.generateCandidatesFromFallback()
  }
  ...
  async executeCycle(trigger) {
    ...
    try {
      ...
      this.latestFindings = results.length ? results : [...FALLBACK_FINDINGS]
  ...
  async collectIntel() {
    ...
    if (!combined.length) {
      this.generateApprovalsAndCandidates([...FALLBACK_FINDINGS], authorizedSources)
  ...
  generateCandidatesFromFallback() {
    this.generateApprovalsAndCandidates([...FALLBACK_FINDINGS], this.authorizedSources)
  }
