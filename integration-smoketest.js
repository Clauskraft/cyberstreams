...
const testMISPIntegration = async () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      logTest('MISP Events API', 'WARN', duration, 'MISP not configured (expected)');
    } else {
  ...
  try {
    ...
    if (response.ok) {
      logTest('MISP Observables API', 'PASS', duration, 'MISP observables working');
    } else if (response.status === 503) {
      logTest('MISP Observables API', 'WARN', duration, 'MISP not configured (expected)');
    } else {
      logTest('MISP Observables API', 'WARN', duration, `HTTP ${response.status} (may be expected)`);
    }
  } catch (error) {
...
const testOpenCTIIntegration = async () => {
  ...
  try {
    const startTime = performance.now();
    const response = await makeRequest(`${BASE_URL}/api/cti/opencti/observables`);
    const duration = performance.now() - startTime;
    
    if (response.ok) {
      const data = await response.json();
      logTest('OpenCTI Observables API', 'PASS', duration, 'OpenCTI integration working');
    } else if (response.status === 503) {
      logTest('OpenCTI Observables API', 'WARN', duration, 'OpenCTI not configured (expected)');
    } else {
      logTest('OpenCTI Observables API', 'FAIL', duration, `HTTP ${response.status}`);
    }
  } catch (error) {
  ...
  for (const test of wigleTests) {
    try {
      ...
      const response = await makeRequest(`${BASE_URL}${test.path}`, {
        method: test.path.includes('search-wifi') || test.path.includes('geocode') ? 'POST' : 'GET',
        body: test.path.includes('search-wifi') ? JSON.stringify({
          ssid: 'test-wifi',
          lat: 55.6761,
          lon: 12.5683,
          radius: 1000
        }) : test.path.includes('geocode') ? JSON.stringify({
      ...
      if (response.ok) {
        ...
        logTest(test.name, 'WARN', duration, 'Wigle Maps not configured (expected)');
      } else {
...
const testIntelScraperIntegration = async () => {
  ...
  for (const test of scraperTests) {
    try {
      ...
      if (response.ok) {
        const data = await response.json();
        logTest(test.name, 'PASS', duration, 'Intel Scraper integration working');
      } else {
        logTest(test.name, 'FAIL', duration, `HTTP ${response.status}`);
      }
    } catch (error) {
...
const testDataFlowIntegration = async () => {
  ...
  try {
    ...
    const knowledgeResponse = await makeRequest(`${BASE_URL}/api/knowledge/stats`);
    if (!knowledgeResponse.ok) {
      throw new Error('Knowledge base not accessible');
    }
    
    // 3. Test search functionality
    const searchResponse = await makeRequest(`${BASE_URL}/api/knowledge/search`, {
      method: 'POST',
      body: JSON.stringify({
        query: 'test search',
        limit: 5
    ...
    if (searchResponse.ok) {
      logTest('Data Flow Integration', 'PASS', duration, 'Complete data flow working');
    } else {
      logTest('Data Flow Integration', 'WARN', duration, 'Search functionality issue');
    }
  } catch (error) {
...
const testIntegrationErrorHandling = async () => {
  ...
  try {
    ...
    if (response.status === 400 || response.status === 503) {
      ...
      logTest('MISP Error Handling', 'WARN', duration, `Unexpected status: ${response.status}`);
    }
  } catch (error) {
...
const generateIntegrationReport = () => {
  ...
  if (results.failed > 0) {
    log('\n🚨 INTEGRATION SMOKETEST FAILED - Critical integration issues detected!', 'red');
    process.exit(1);
  } else if (results.warnings > 0) {
    log('\n⚠️  INTEGRATION SMOKETEST PASSED with warnings', 'yellow');
    process.exit(0);
  } else {
    log('\n✅ INTEGRATION SMOKETEST PASSED - All integrations working!', 'green');
    process.exit(0);
  }
};
