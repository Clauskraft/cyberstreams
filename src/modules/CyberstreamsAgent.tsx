...
import {
  Bot, Shield, AlertTriangle, Clock, Filter, Search,
  Globe, Eye, Target, Zap, Calendar,
  TrendingUp, Database, Activity, Settings
} from 'lucide-react'
...
const CyberstreamsAgent = () => {
    ...
    const fallbackFindings: AgentFinding[] = [
      {
        ...
        description: 'CFCS.dk advarer om aktiv udnyttelse af Remote Code Execution sårbarhed i Exchange Server. Påvirker danske virksomheder.',
        category: 'threat',
        severity: 'critical',
        source: 'CFCS.dk',
      ...
      },
      {
        ...
        description: 'Ny lovgivning om obligatorisk incident rapportering for kritisk infrastruktur. Træder i kraft 1. januar 2026.',
        category: 'politics',
        severity: 'medium',
      ...
      },
      {
        ...
        title: 'Ransomware gruppe LockBit rammer danske energiselskaber - politisk respons',
        description: 'LockBit 3.0 kampagne mod dansk energisektor. Energiminister indkalder til krisemøde om kritisk infrastrukturbeskyttelse.',
        category: 'both',
        severity: 'high',
      ...
      },
      {
        ...
        description: 'Unverified reports of Chrome zero-day being sold on Russian underground forum. No official vendor confirmation yet.',
        category: 'threat',
        severity: 'high',
        source: 'XSS Forum',
        sourceType: 'darkweb',
      ...
      },
      {
        ...
        title: 'EU Digital Services Act påvirker danske cybersikkerhedsudbydere',
        description: 'Nye DSA-regler kræver enhanced incident reporting. Danske security vendors skal tilpasse compliance processer.',
        category: 'politics',
        severity: 'medium',
        source: 'Version2',
        sourceType: 'media',
      ...
      },
      {
        ...
        title: 'APT28 campaign targeting Nordic government entities with new malware',
        description: 'CERT-EU reports sophisticated campaign using novel persistence mechanisms. Danish government entities advised to implement additional monitoring.',
        category: 'threat',
        severity: 'high',
        source: 'CERT-EU',
        sourceType: 'government',
      ...
      },
      {
        ...
        title: 'Regeringen fremlægger national AI-sikkerhedsstrategi med cybertrusselsfokus',
        description: 'Ny strategi adresserer AI-drevne cyberangreb og kræver særlige sikkerhedsforanstaltninger for AI-systemer i kritisk infrastruktur.',
        category: 'both',
        severity: 'medium',
  ...
  useEffect(() => {
    ...
    if (sourceFilter !== 'all') {
      filtered = filtered.filter(finding => finding.sourceType === sourceFilter)
    }
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'threat': return 'bg-red-500/20 text-red-300'
      case 'politics': return 'bg-blue-500/20 text-blue-300'
    ...
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-center">
          ...
          <p className="text-gray-400">CYBERSTREAMS-AGENT analyserer sources...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-purple-600/10 to-blue-600/10 rounded-lg p-6 border border-purple-500/30">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-400">Deterministisk threat-intel-agent for Danmark &amp; EU</p>
          </div>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-6 gap-3 text-sm">
          ...
          <div className={`${scraperStatus.isRunning ? 'text-green-400' : 'text-gray-400'}`}>
            <Activity className="w-4 h-4 inline mr-1" />
            Scraper: {scraperStatus.isRunning ? 'RUNNING' : 'STOPPED'} ({scraperStatus.activeSources}/{scraperStatus.totalSources})
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-900 rounded-lg border border-gray-700">
        <nav className="flex">
          {[
            { id: 'trusler' as const, label: 'Trusler', count: stats.threatsActive, icon: Shield },
            { id: 'politik' as const, label: 'Politik', count: stats.politicsActive, icon: TrendingUp },
            { id: 'begge' as const, label: 'Begge', count: findings.filter(f => f.category === 'both').length, icon: Zap }
          ].map((tab) => (
            <button
              key={tab.id}
              ...
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className={`text-xs px-2 py-1 rounded-full ${getCategoryColor(finding.category)}`}>
                      {finding.category === 'threat' ? 'TRUSSEL' : 
                       finding.category === 'politics' ? 'POLITIK' : 'BEGGE'}
                    </span>
                    <span className={`text-xs px-2 py-1 rounded-full ${
                      finding.verified ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'
                    }`}>
                    ...
                    <div className="mb-3">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {finding.cves.map((cve, idx) => (
                          <code key={idx} className="text-xs px-2 py-1 bg-red-900/30 text-red-300 rounded">
                            {cve}
                          </code>
                        ))}
                      </div>
                    ...
                    <div className="mb-3">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {finding.iocs.map((ioc, idx) => (
                          <code key={idx} className="text-xs px-2 py-1 bg-yellow-900/30 text-yellow-300 rounded">
                            {ioc}
                          </code>
                        ))}
                      </div>
