...
class MemoryVectorStore {
  ...
  search(namespace: string, query: string, limit = 5) {
    ...
    for (const document of this.store.values()) {
      ...
      const dot = document.embedding.reduce(
...
const seedArticles: SignalStreamArticle[] = [
  {
    id: "sig-art-001",
    title:
      "CloudKey® bruges som reference i ny EU-vejledning om datasuverænitet",
    summary:
...
const SignalStream = () => {
  ...
  const [sessionTrace, setSessionTrace] =
    useState<SessionTrace>(createInitialTrace);
  ...
  const [osintCommands, setOsintCommands] = useState<
  ...
  useEffect(() => {
    const fetchSignalStreamData = async () => {
      try {
        ...
        if (data.success && data.data && data.data.length > 0) {
          ...
          const transformedArticles: SignalStreamArticle[] = data.data.map(
    ...
    // Load OSINT commands for dropdown
    (async () => {
      try {
        const r = await fetch("/api/osint/commands");
        const j = await r.json();
        if (j?.success && Array.isArray(j.data)) {
          setOsintCommands(j.data);
          setSelectedCommand(j.data[0]?.id || "");
        }
      } catch (e) {
        // ignore
      }
  ...
  const pushTraceEvent = (
    event: Omit<TraceEvent, "id" | "timestamp"> & { timestamp?: string }
  ) => {
  ...
  const handleIngest = () => {
    ...
    pushTraceEvent({
      type: "compose",
      label: "Datasæt indlæst",
      details: { mode: "manual ingest" },
    });
    ...
    setTimeout(() => {
      vectorStore.upsert(
        articles.map((article) => ({
          id: `${article.id}-full`,
          namespace: "articles",
          text: `${article.title} ${article.summary} ${article.analysis.join(
            " "
          )} ${article.implications.join(" ")}`,
          metadata: { focusLane: article.focusLane, tags: article.tags },
        }))
      );
  ...
  const handleCompose = () => {
    ...
    pushTraceEvent({
      type: "compose",
      label: "SignalStream genererer artikler",
      details: { promptName },
    });

    setTimeout(() => {
      const focus = activeArticle?.focusLane ?? "Datasuverænitet";
      const nearest = vectorStore.search(
        "articles",
        `${focus} ${config.language}`,
        3
      );
      ...
      const composedArticles = articles.map((article, index) => {
        ...
        return {
          ...
          evidence: {
            ...
            bm25Score: Number(
              (
                article.evidence.bm25Score * 0.95 +
                0.05 * Math.random()
              ).toFixed(2)
            ),
            freshnessHours: Math.max(
              2,
              Math.round(article.evidence.freshnessHours - 1)
            ),
            domainAuthority: article.evidence.domainAuthority,
            citationCoverage:
              article.sources.length / Math.max(article.sources.length, 1),
          },
          why: [
            `Vector-score ${Number(
              vectorScore.toFixed(2)
            )} baseret på systemprompten`,
            ...article.why.slice(1),
          ],
        };
      });
  ...
  const runOsintCommand = async () => {
    if (!selectedCommand) return;
    setExecuting(true);
    try {
      const r = await fetch("/api/osint/execute", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ commandId: selectedCommand }),
      });
      const j = await r.json();
      pushTraceEvent({
        type: "compose",
        label: `OSINT: ${selectedCommand}`,
        details: { result: j?.message || j?.error },
      });
    } catch (e) {
      pushTraceEvent({
        type: "compose",
  ...
  const handleExpandScope = (
    strategy: "broaden" | "narrow" | "adjacent" | "temporal"
  ) => {
    const query = `${strategy} ${
      activeArticle?.focusLane ?? "datasuverænitet"
    }`;
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-300">
                Differentieret Pulse-klon med evidenskriterier, fokusstier og
                horisontskifte.
              </p>
            </div>
          </div>
          <div className="flex flex-wrap gap-2 text-xs text-gray-300">
            ...
            <span className="px-3 py-1 bg-gray-800/60 rounded-full flex items-center gap-2">
              ...
              {config.freshnessDays} dages friskhed
            </span>
          </div>
        </div>
      </Card>
        ...
        <div className="space-y-6">
          <Card>
            <div className="flex items-start justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-gray-400">
                  Indlæs eller tilpas den strategiske profil. Promptnavnet
                  bruges i sessionstraces.
                </p>
              </div>
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              <Button
                onClick={() => setPromptName("SignalStream Default")}
                className="bg-cyber-blue/80"
              >
                Gem navn
              </Button>
              <Button
          ...
          <Card>
            <div className="flex flex-wrap items-center justify-between gap-4">
              <div>
                ...
                <p className="text-sm text-gray-400">
                  Kør ingest for at placere artikler, kilder og trusler i
                  SignalStreams vektorlager.
                </p>
              </div>
              <div className="flex items-center gap-2">
                ...
                <Button
                  onClick={runOsintCommand}
                  disabled={executing}
                  className="flex items-center gap-2"
                  data-testid="osint-run-now"
                >
                  <RefreshCw
                ...
                >
                  <RefreshCw
                    className={`w-4 h-4 ${
                      ingestStatus === "running" ? "animate-spin" : ""
                    }`}
                  />
                  Ingest
                </Button>
            ...
            {selectedCommand && (
              <div
                ...
                data-testid="osint-command-description"
              >
                {
                  osintCommands.find((c) => c.id === selectedCommand)
                    ?.description
                }
              </div>
            )}
          ...
          <Card>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div>
                <h2 className="text-xl font-semibold flex items-center gap-2">
                  <Sparkles className="w-5 h-5 text-cyber-blue" /> Komponér
                  artikler
                </h2>
                <p className="text-sm text-gray-400">
                  SignalStream kombinerer prompten med vektorlageret.
                </p>
              </div>
              <Button
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="text-xs text-gray-400 text-right">
                        ...
                        <span>
                          {new Date(article.createdAt).toLocaleString("da-DK")}
                        </span>
                      </div>
                    </div>
                    <div className="grid gap-3 md:grid-cols-[1fr,1fr]">
                      ...
                      <div className="text-xs text-gray-300 space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span>
                            Evidensscore:{" "}
                            {article.evidence.vectorScore.toFixed(2)}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          <History className="w-4 h-4 text-amber-400" />
                          <span>
                            Friskhed: {article.evidence.freshnessHours} timer
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          <span>
                            Domæneautoritet:{" "}
                            {article.evidence.domainAuthority.toFixed(2)}
                          </span>
                        </div>
                        <div className="flex items-center gap-2">
                    ...
                    <div className="text-xs text-gray-400 flex flex-wrap gap-2">
                      {article.sources.map((source) => (
                        <span
                          key={source.id}
                          className="px-2 py-1 bg-gray-800/80 rounded"
                        >
              ...
              <div className="flex flex-col gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-2">
                    {activeArticle.images[1]?.caption}
                  </p>
                </div>
              </div>
            </Card>
          ...
          <Card>
            <div className="flex items-center justify-between">
              <h2 className="text-xl font-semibold flex items-center gap-2">
                <GitBranch className="w-5 h-5 text-cyber-blue" /> Horisontskifte
                forslag
              </h2>
              <Button
            ...
            <div className="mt-4 space-y-3">
              {suggestions.map((suggestion) => (
                <div
                  key={suggestion}
                  className="p-3 rounded-lg bg-gray-900 border border-gray-800 text-sm text-gray-300"
                >
                  {suggestion}
                </div>
              ))}
            ...
            <div className="mt-4 space-y-3 text-sm text-gray-300">
              <div className="flex items-center justify-between">
                ...
                <Button
                  ...
                  onClick={() =>
                    setConfig((prev) => ({
                      ...prev,
                      language: prev.language === "da" ? "en" : "da",
                    }))
                  }
                >
              ...
              <div>
                ...
                <div className="mt-2 space-y-2">
                  {["enisa.europa.eu", "digst.dk", "cfcs.dk", "tdc.dk"].map(
                    (domain) => (
                      <label
                        key={domain}
                ...
                <label className="flex items-center gap-2 text-xs">
                  <input
                    ...
                    onChange={() =>
                      setConfig((prev) => ({
                        ...prev,
                        requireCitations: !prev.requireCitations,
                      }))
                    }
                    className="accent-cyber-blue"
                  />
                ...
                <label className="flex items-center gap-2 text-xs">
                  <input
                    ...
                    onChange={() =>
                      setConfig((prev) => ({
                        ...prev,
                        rejectLowTrust: !prev.rejectLowTrust,
                      }))
                    }
                    className="accent-cyber-blue"
                  />
          ...
          <Card>
            ...
            {activeArticle ? (
              <div className="mt-4 space-y-3 text-sm text-gray-300">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-cyan-400 font-semibold">
                    {activeArticle.evidence.vectorScore.toFixed(2)}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span>BM25</span>
                  <span className="text-cyan-400 font-semibold">
                    {activeArticle.evidence.bm25Score.toFixed(2)}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span>Friskhed (timer)</span>
                  <span className="text-amber-400 font-semibold">
                    {activeArticle.evidence.freshnessHours}
                  </span>
                </div>
                <div className="flex items-center justify-between">
                  <span>Domæneautoritet</span>
                  <span className="text-emerald-400 font-semibold">
                    {activeArticle.evidence.domainAuthority.toFixed(2)}
                  </span>
                </div>
                <div className="flex items-center justify-between">
              ...
              <p className="mt-4 text-sm text-gray-400">
                Vælg en artikel for at se evidenskriterier.
              </p>
            )}
          </Card>
                ...
                {traceHistory.map((trace) => (
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <span>
                        {new Date(trace.startedAt).toLocaleDateString("da-DK")}
                      </span>
                    </div>
